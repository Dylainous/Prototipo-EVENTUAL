...
class _MemberFormPageState extends State<MemberFormPage> {
            ...
            child: Column(children: [
              if (!_isEditing) ...[
                _field(
                    ...
                    // Añadimos los formateadores aquí:
                    inputFormatters: [
                      FilteringTextInputFormatter.digitsOnly, // 100% libre de letras/símbolos
                      LengthLimitingTextInputFormatter(10),   // Tope máximo de 10 caracteres
                    ],
                    validator: (v) {
                      if (v == null || v.trim().isEmpty) {
                        return 'La cédula es requerida';
                      }
                      if (!_validarCedula(v.trim())) {
  ...
  Widget _field(
    ...
    bool obscure = false,
    int maxLines = 1,
    List<TextInputFormatter>? inputFormatters, // <-- 1. Agregamos este parámetro
  }) {
    return TextFormField(
      ...
      maxLines: maxLines,
      validator: validator,
      inputFormatters: inputFormatters, // <-- 2. Se lo pasamos al TextFormField
...
bool _validarCedula(String cedula) {
  // 1. Validaciones básicas de longitud y caracteres
  ...
  // 4. Algoritmo de coeficientes de validación (2, 1, 2, 1, 2, 1, 2, 1, 2)
  final coeficientes = [2, 1, 2, 1, 2, 1, 2, 1, 2];
  int suma = 0;

  for (int i = 0; i < 9; i++) {
    int valor = int.parse(cedula.substring(i, i + 1)) * coeficientes[i];
    if (valor >= 10) valor -= 9; // Si el producto es >= 10, se le resta 9
    suma += valor;
  }

  // 5. Verificar contra el décimo dígito
  final digitoVerificador = int.parse(cedula.substring(9, 10));
  final residuo = suma % 10;
  final digitoCalculado = residuo == 0 ? 0 : 10 - residuo;

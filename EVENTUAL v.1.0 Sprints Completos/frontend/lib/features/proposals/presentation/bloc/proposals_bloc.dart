...
class ProposalsBloc extends Bloc<ProposalsEvent, ProposalsState> {
  ...
  Future<void> _onLoadMine(ProposalsLoadMine e, Emitter<ProposalsState> emit) async {
    ...
    try {
      final body = await _api.get(ApiConstants.myProposals);
      final list = (body['propuestas'] as List)
          .map((j) => ProposalEntity.fromJson(j as Map<String, dynamic>))
          .toList();
  ...
  Future<void> _onSubmit(
    ...
    emit(ProposalsLoading());
    try {
      final body = await _api.post(ApiConstants.proposals, {
      ...
      // Ahora usamos el mensaje de error que lanza el ApiClient
      emit(ProposalsError(e.toString().replaceFirst('Exception: ', '')));
    }
  }
}